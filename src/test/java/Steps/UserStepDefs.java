...
public class UserStepDefs {
    ...
    public void verifyTheResponseAfterLoginIntoBookStoreShouldAndSuccess(int statusCode,String condition) {
        ...
        switch (condition)
        {
            ...
            case "missingParam":
                Assert.assertEquals(bookStoreData.getLogInResponse().getStatusCode(), 422, "Status code is not 422");
    ...
    public void editTheNameOfTheBookAddedAndVerifyTheResponseAfterUpdate(String editAction) {
        ...
        if(editAction.equalsIgnoreCase("noAccessToken")){
            bookStoreData.setEditBookResponse(BooksApi.editBook(bookDetails,null));
        }
        else {
            bookStoreData.setEditBookResponse(BooksApi.editBook(bookDetails,bookStoreData.getAccessToken()));
        }
    ...
    public void getTheDetailsOfTheParticularBookUsingBookIdGeneratedWhileCreating() {
      bookStoreData.setBookInfoById(BooksApi.getBookInfoById(bookDetails, bookStoreData.getAccessToken()));
    ...
    public void verifyTheBookDetailsAreFetchedProperlyInTheResponseByBookId() {
        Assert.assertEquals(bookStoreData.getBookInfoById().getBody().jsonPath().get("name"),bookDetails.get("bookName"),"Book name  mismatch");
        Assert.assertEquals(bookStoreData.getBookInfoById().getBody().jsonPath().get("author"),bookDetails.get("author"),"Author name mismatch");
        Assert.assertEquals(bookStoreData.getBookInfoById().getBody().jsonPath().get("published_year"),bookDetails.get("published_year"),"Published year mismatch");
        Assert.assertEquals(bookStoreData.getBookInfoById().getBody().jsonPath().get("book_summary"),bookDetails.get("book_summary"),"Book summary  mismatch");
        Assert.assertEquals(bookStoreData.getBookInfoById().getBody().jsonPath().get("id"),bookDetails.get("createdBookId"),"Book id is different to the request");
    }
    ...
    public void verifyTheBookDetailsShouldNotBeFetchedProperlyInTheResponseForDeletedBookId() {
        Assert.assertEquals(bookStoreData.getBookInfoById().getBody().jsonPath().get("detail"),"Book not found","Book details should not be fetched for deleted");
    }
    ...
    public void deleteTheAddedBookInTheBookStoreUsingBookIdAndVerifyTheResponse() {
       bookStoreData.setDeleteBookResponse(BooksApi.deleteBookById(bookDetails.get("createdBookId").toString(), bookStoreData.getAccessToken()));
    }
    ...
    public void sign_up_to_the_book_store_as_the_new_user_with_email_and_password() {
        // This step can be used to initialize or reset any state if needed before sign up
        // For now, no action is required as the actual sign up is handled in the next step
    }
